...
while true do
  ...
  if(currentTime > nextLogTime) then
    ...
    nextLogTime = socket.gettime() + 10
  end

  if(currentTime > nextOrderTime) then
    beforeAction = socket.gettime()
    client:get(joinTwoStrings(myId,"order", ":"))
    ...
    nextOrderTime = socket.gettime() + 60
  end
  ...
  if(currentTime > nextActiveTime) then
    ...
    nextActiveTime = socket.gettime() + 300
  end
  ...
  if(currentTime > nextSyncSetTime) then
    ...
    countSyncSetTime = countSyncSetTime + 1

    nextSyncSetTime = socket.gettime() + 1
  end
  ...
  if(currentTime > nextSyncGetTime) then
    ...
    for i = 1, math.random(1, 5) do
      ...
      countSyncGetTime = countSyncGetTime + 1

      nextSyncGetTime = socket.gettime() + 1
    end
  end

  if(currentTime > nextSetValues) then
    client:set(joinTwoStrings(myId,"values", ":"), averageLogTime .. "\t" .. averageOrderTime .. "\t" .. averageActiveTime .. "\t" .. averageSyncSetTime .. "\t" .. averageSyncGetTime)
    nextOrderTime = socket.gettime() + 10
  end
