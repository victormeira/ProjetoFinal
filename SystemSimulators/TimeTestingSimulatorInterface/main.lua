...
local MAX_CARS = 1000
local SIMULATION_END = 4 * MAX_CARS
local totalPassedCars = 0
local testType = tostring(MAX_CARS) .. "CARS_DCA"
...
function addCarsToRoad(carsList, n)
	...
	for i=1, n do
		...
		totalPassedCars = totalPassedCars + 1

		if totalPassedCars > SIMULATION_END + maxCarsInRoad then
			love.event.quit()
		end
