...
while true do
  ...
  if(currentTime > nextaggregateTime) then
    ...
    beforeAction = socket.gettime()
    client:set(joinTwoStrings(math.random(1, #activeIds + 1), "aggregate", ":"), generateRandomString())
    ...
    nextaggregateTime = socket.gettime() + 2
  end

  if(currentTime > nextSetValues) then
    client:set("manager:values", averageLogTime .. "\t" .. averageaggregateTime .. "\t" .. averageActiveTime)
